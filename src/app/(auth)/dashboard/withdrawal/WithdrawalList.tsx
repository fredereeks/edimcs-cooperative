...
export default function WithdrawalList({ withdrawalData }: { withdrawalData: WithdrawalProps[] }) {
                        ...
                        <thead>
                            <tr>
                                <th colSpan={user.type === "Admin" ? 6 : 5}>
                                    <TableSearch title='WITHDRAWAL' key={'72088234'} handleSearch={handleSearch} inputRef={inputRef}>
                                    ...
                                    <tr key={withdrawal.id} className='hover:bg-slate-50 dark:hover:bg-slate-900/30'>
                                        ...
                                        <td className="align-middle">
                                            <div className="flex justify-center items-center align-middle mx-auto whitespace-nowrap">
                                                <div className={`${withdrawal.verdict === "Rejected" ? 'bg-[#f34e7c20] dark:bg-[#faf0f3de] text-danger' : withdrawal.verdict === "Accepted" ? 'bg-[#f34e7c20] dark:bg-[#faf0f3de] text-danger' : 'bg-slate-100 text-slate-500'} text-xs py-[.1rem] sm:py-1 px-3 rounded-sm font-medium`}>-&#8358;{withdrawal.amount.toLocaleString()}</div>
                                            </div>
                                        </td>
                                        <td className="align-middle">
                                            <div className="flex justify-center items-center gap-[.2rem] align-middle text-slate-400 text-xs py-[.1rem] sm:py-1">
                                                <FaClock className="text-inherit mt-[.1rem]" /> <p className="">{withdrawal.createdAt}</p>
                                            </div>
                                        </td>
                                        <td className="align-middle">
                                            <div className="flex justify-center gap-2">
                                                <div className={`${withdrawal.verdict === "Rejected" ? 'bg-[#f34e7c20] dark:bg-[#faf0f3de] text-danger' : withdrawal.verdict === "Accepted" ? 'bg-teal-100 text-teal-500' : 'bg-slate-100 text-slate-500'} text-xs py-[.1rem] sm:py-1 px-3 rounded-sm font-medium`}>{withdrawal.verdict}</div>
                                            </div>
                                        </td>
                                        <td className="align-middle">
                                            <div className="flex justify-center gap-2">
                                                <div className={`${withdrawal.status === "Rejected" ? 'bg-[#f34e7c20] dark:bg-[#faf0f3de] text-danger' : 'bg-slate-100 text-slate-500'} text-xs py-[.1rem] sm:py-1 px-3 rounded-sm font-medium`}>{withdrawal.status}</div>
                                            </div>
                                        </td>
                                        {user.type === "Admin" &&
                                            <td className="align-middle">
                                ...
                                <div className='flex-1 flex flex-col justify-center w-full'>
                                    <div className="flex justify-between items-center gap-4 text-slate-600">
                                        ...
                                        <div className="flex justify-center items-center gap-[.2rem] align-middle dark:text-slate-100 text-[.6rem]">
                                            <FaCalendarAlt className="text-inherit opacity-60" /> <p className="">{selectedWithdrawal?.createdAt}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <form ref={formRef} className="flex flex-col font-normal">
                        <div className="flex justify-between items-center gap-2 border-y border-y-slate-300 py-2 my-2 text-slate-700">
                            ...
                            <div className="flex overflow-x-hidden relative w-[8rem] max-w-[8rem] border border-gray-300 rounded-md px-2 py-0">
                                <input type="number" required min={500} max={user?.balance} ref={amountRef} name='payback' placeholder={`Minimum of ₦500`} className="relative outline-none py-2 pl-2 pr-4 text-gray-600 text-xs placeholder-opacity-70 font-normal flex w-[10rem] bg-transparent focus-within:bg-transparent focus:bg-transparent" />
                            </div>
                        </div>
                        <button type="submit" className="py-2 px-4 sm:px-8 bg-danger text-white text-[.6rem] text-xs rounded-md hover:bg-danger/90 cursor-pointer" disabled={loading}>Request Withdrawal</button>
                    </form>
                </div>
            </Modal>
