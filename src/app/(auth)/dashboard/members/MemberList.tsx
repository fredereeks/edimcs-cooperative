...
export default function MemberList({ memberData }: { memberData: MemberProps[] }) {
    ...
    return (
        <>
            <section className="relative flex flex-col gap-2 p-4 bg-white dark:bg-slate-200 shadow-slate-200 dark:shadow-black shadow-md rounded-lg">
                                    ...
                                    <tr onClick={() => handleClick(member.id)} key={member.id}>
                                        <td>
                                            <div className="max-w-sm w-max flex items-center gap-2 cursor-pointer">
                                                ...
                                                <div>
                                                    <h5 className="text-xs font-medium leading-tight whitespace-nowrap flex items-center">{member?.firstname} {member?.middlename} {member?.lastname} <span className="text-[.4rem] bg-slate-200/50 p-[.2rem] px-[.3rem] rouded-md text-slate-600 uppercase ml-2">{member.type}</span></h5>
