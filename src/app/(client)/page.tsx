...
export default function Home() {
  return (
    <main className="flex min-h-screen flex-col">
      <article  data-aos-duration="1000" data-aos="zoom-out-up" className="relative py-20 px-4 bg-slate-600 after:bg-gradient-t-b after:from-slate-500 after:to-primary after-overlay">
        <Image data-aos-duration="4000" data-aos-delay="1000" data-aos="fade-left" src={edimcs_piggyvest} alt='Edimcs Money Calculator Image' fill={true} className="absolute w-1/2 left-1/2 top-0 opacity-50 object-cover" />
        <div className="backdrop-blur-md relative z-20 container mx-auto py-20 h-screen max-h-screen grid sm:grid-cols-2 gap-4 text-white">
          <div className="flex flex-col gap-2 justify-center">
            <h2 data-aos-duration="4000" data-aos-delay="1000" data-aos="zoom-out-up" className="text-3xl sm:text-4xl font-semibold max-w-md sm:max-w-lg leading-tight">Saving you Money before you know it. Literally!</h2>
            <p data-aos-duration="4000" data-aos-delay="1000" data-aos="zoom-out-down" className="text-base sm:text-lg opacity-80 font-medium max-w-lg leading-loose text-justify py-2">We are a smart saving cooperative company that helps you set your money apart for future use. We provide guaranteed withdrawal, real-time tracking and incredible potential for your financial life.</p>
            <div data-aos-duration="4000" data-aos-delay="1000" data-aos="fade-down-right" className="py-3 flex gap-2 sm:gap-4 items-center">
          ...
          <div className="relative h-full hidden sm:flex">
            <Image src={edimcs_phonecalculator} alt='Edimcs Money Calculator Image' fill={true} className="overlay left-0 top-0 object-cover" />
          </div>
        </div>
      </article>
      <section  data-aos-duration="1000"  data-aos="zoom-out-up"  className="py-20 px-4 bg-white relative">
        <div className="container mx-auto flex flex-col gap-6 md:flex-row py-10 md:py-20">
          <div data-aos-duration="4000" data-aos-delay="1000" data-aos="fade-down-left" className="relative flex-1 flex justify-end">
            <div data-aos-duration="4000" data-aos-delay="1000" data-aos="fade-down-right" className="relative min-h-[250px] sm:h-[300px] md:h-full p-10 w-full overflow-hidden">
          ...
          <div className="py-0 md:py-20 flex-1 flex flex-col gap-2 sm:justify-center text-justify">
            <h3 data-aos-easing="ease-in-sine" data-aos-duration="1000" data-aos="fade-left" className="text-slate-700 text-4xl sm:text-5xl lg:text-6xl text-left leading-snug font-bold max-w-md sm:max-w-lg w-full relative left-1/2 -translate-x-1/2 pb-4">A <span className="text-red-500">Cooperative</span> with a Pedigree.</h3>
            <p data-aos-easing="ease-in-sine" data-aos-duration="1000" data-aos="fade-up-left" style={{ lineHeight: 1.9 }} className="text-slate-500 text-xs sm:text-sm leading-loose font-light max-w-md sm:max-w-lg w-full relative left-1/2 -translate-x-1/2">Enlightenment Drive Initiative Co-operative Society (EDIMCS)  largest cooperative organization among cooperatives in Nigeria and in the world, with the goal of achieving a first-world Nigeria with empowerment and housing for low-income and small and medium enterprises as our primary goal.</p>
            <p data-aos-easing="ease-in-sine" data-aos-duration="1000" data-aos="fade-right" style={{ lineHeight: 1.9 }} className="text-slate-500 text-xs sm:text-sm leading-loose font-light max-w-md sm:max-w-lg w-full relative left-1/2 -translate-x-1/2">Our aim at EDIMCS is to level up the playing field in the cooperative industry through the power of team, cooperation, and networking.</p>
          </div>
        </div>
      </section>
          ...
          <div data-aos-easing="ease-in-sine" data-aos-duration="1000" data-aos="zoom-out-up" className="relative z-20 container mx-auto py-5 grid sm:grid-cols-2 md:grid-cols-3 text-white">
            {
              [
                ...
                <div data-aos-easing="ease-in-sine" data-aos-duration="3000" data-aos="zoom-out-up" key={id.toString()} className={`${i === 0 ? 'bg-zinc-800 sm:col-span-2 md:col-span-1' : i === 1 ? 'bg-zinc-800/80' : 'bg-zinc-800/10'} p-7 flex flex-col gap-2`}>
                  <span data-aos-easing="ease-in-sine" data-aos-duration="1000" data-aos="fade-up" className={`text-3xl sm:text-4xl flex-shrink-0 pt-5 ${i === 0 ? 'text-slate-100' : i === 1 ? 'text-slate-300' : 'text-slate-700'}`}>{icon}</span>
                  <div className="py-5 flex flex-col gap-2">
                    <h3 data-aos-easing="ease-in-sine" data-aos-duration="1000" data-aos="fade-down" className={`text-lg sm:text-xl font-medium sm:font-semibold max-w-lg leading-tight ${i === 0 ? 'text-slate-100' : i === 1 ? 'text-slate-300' : 'text-slate-700'}`}>{title}</h3>
                    <p data-aos-easing="ease-in-sine" data-aos-duration="1000" data-aos="zoom-out-right" style={{ lineHeight: 2 }} className={`text-xs sm:text-sm font-thin leading-loose text-justify ${i === 0 ? 'text-slate-100' : i === 1 ? 'text-slate-300' : 'text-slate-700'}`}>{text}</p>
                  </div>
                </div>
              )))
            }
