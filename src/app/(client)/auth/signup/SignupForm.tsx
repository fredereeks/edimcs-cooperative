...
export default function SignupForm({ handleLogin }: { handleLogin: (formData: FormData) => Promise<LoginFormDataProps | null> }) {
    ...
    return (
        <>
            <form ref={formRef} action={handleSubmit} className="py-5 flex flex-col gap-3 px-4 sm:px-0">
                ...
                <button type="submit" disabled={loading} className="rounded-full py-2 px-5 md:px-8 w-max bg-danger text-white text-sm text-center flex-1 cursor-pointer flex items-center gap-2 mt-2"><FaMoneyCheck className="text-sm text-inherit" />{loading ? 'Processing...' : 'Signup'}</button>
            </form>
